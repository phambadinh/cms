...
import { BookOpen, Trophy, Target, TrendingUp, Users, Eye, Globe, GraduationCap } from "lucide-react";
...
function LearningProgress() {
  ...
  const getStat = (label) => dashboard?.stats?.find((s) => s.label === label)?.value || "0";
    ...
    <div className="learning-progress-page">
      ...
      <div className="progress-course-card">
        ...
        <div className="progress-bar">
          <div className="progress-fill" style={{ width: `${Math.min(parseFloat(averageProgress || 0), 100)}%` }} />
        </div>

        <p>
          Bạn đã hoàn thành <strong>{completedLessons}</strong> bài học.
        </p>
      </div>
          ...
          {dashboard.courses.map((course) => (
            <div key={course.id} className="progress-course-card">
              ...
              <div className="course-meta">
                ...
                <div className="meta-item">
                  <Eye size={18} />
                  <span>{course.viewCount} lượt xem</span>
                </div>

                <div className="meta-item">
                  <Globe size={18} />
                  <span>{course.published ? "Đã công bố" : "Chưa công bố"}</span>
                </div>
              </div>
            </div>
          ))}
